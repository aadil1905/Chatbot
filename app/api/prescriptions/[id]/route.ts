...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const prescription = await prisma.prescription.update({ where: { id }, data: { patientId: data.patientId, prescribedOn: localDate(data.prescribedOn), diagnosis: data.diagnosis || null, instructions: data.instructions || null, medicines: data.medicines } });
