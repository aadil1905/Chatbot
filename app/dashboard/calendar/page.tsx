...
export default async function CalendarPage({ searchParams }: { searchParams: Promise<{ week?: string }> }) {
  ...
  const appointments = await prisma.appointment.findMany({ where: { clinicId: user.clinicId, archivedAt: null, appointmentDate: { gte: start, lt: end } }, include: { provider: true, chair: true }, orderBy: [{ appointmentDate: "asc" }, { appointmentTime: "asc" }] });
