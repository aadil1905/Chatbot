...
export default async function CalendarPage({ searchParams }: { searchParams: Promise<{ week?: string }> }) {
  ...
  const appointments = await prisma.appointment.findMany({ where: { clinicId: user.clinicId, appointmentDate: { gte: start, lt: end } }, orderBy: [{ appointmentDate: "asc" }, { appointmentTime: "asc" }] });
