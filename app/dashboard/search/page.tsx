...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const [patients, appointments, leads, invoices] = q ? await Promise.all([
    prisma.patient.findMany({ where: { clinicId: user.clinicId, OR: [{ fullName: { contains: q, mode: "insensitive" } }, { phone: { contains: q } }] }, take: 6 }),
    prisma.appointment.findMany({ where: { clinicId: user.clinicId, OR: [{ patientName: { contains: q, mode: "insensitive" } }, { phone: { contains: q } }, { treatment: { contains: q, mode: "insensitive" } }] }, take: 6 }),
    prisma.lead.findMany({ where: { clinicId: user.clinicId, OR: [{ fullName: { contains: q, mode: "insensitive" } }, { phone: { contains: q } }] }, take: 6 }),
    prisma.invoice.findMany({ where: { patient: { clinicId: user.clinicId, OR: [{ fullName: { contains: q, mode: "insensitive" } }, { phone: { contains: q } }] } }, include: { patient: true }, take: 6 }),
  ]) : [[], [], [], []];
