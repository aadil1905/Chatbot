...
export default async function ClinicalWorkspacePage() {
  ...
  const patients = await prisma.patient.findMany({ where: { clinicId: user.clinicId }, include: { dentalChartEntries: { select: { id: true } }, clinicalRecords: { orderBy: { visitDate: "desc" }, take: 1 } }, orderBy: { updatedAt: "desc" }, take: 50 });
