...
function label(status: string) {
  return status === "Paid"
    ...
    : status === "Partially Paid"
      ...
      : status === "Overdue" ? "bg-rose-100 text-rose-800" : "bg-slate-100 text-slate-700";
}
...
export default async function BillingPage() {
  ...
  const overdue = invoices.filter((invoice) => invoice.dueDate && invoice.dueDate < new Date() && invoice.payments.reduce((sum, payment) => sum + payment.amount, 0) < invoice.totalAmount);
  const overdueAmount = overdue.reduce((sum, invoice) => sum + invoice.totalAmount - invoice.payments.reduce((paid, payment) => paid + payment.amount, 0), 0);
  ...
  return (
    <div className="dashboard-list-page mx-auto max-w-7xl space-y-6">
      <header className="dashboard-page-header flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">Revenue</p>
          <h1 className="mt-2 text-3xl font-bold">Invoices and collections</h1>
          <p className="mt-1 text-muted-foreground">Track billed value, payments received, and the patient balances that need recovery.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Card><CardContent className="pt-5"><p className="text-sm text-muted-foreground">Overdue recovery</p><p className="mt-1 text-2xl font-bold text-rose-700">₹{overdueAmount.toLocaleString("en-IN")}</p><p className="mt-1 text-xs text-muted-foreground">{overdue.length} past-due invoice{overdue.length === 1 ? "" : "s"}</p></CardContent></Card>
      </div>
          ...
          {invoices.length === 0 ? (
            ...
            <div className="divide-y">
              {invoices.map((invoice) => {
                const paid = invoice.payments.reduce((sum, payment) => sum + payment.amount, 0);
                const status = paid === invoice.totalAmount ? "Paid" : invoice.dueDate && invoice.dueDate < new Date() ? "Overdue" : paid > 0 ? "Partially Paid" : "Unpaid";
                return (
                  <article key={invoice.id} className="flex flex-col gap-3 p-5 hover:bg-muted/30 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex shrink-0 flex-wrap items-center gap-3 sm:justify-end sm:text-right">
                      <div>
                        ...
                        <span className={`mt-2 inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${label(status)}`}>{status}</span>
                      </div>
