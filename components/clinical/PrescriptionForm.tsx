...
type EditingPrescription = { id: number; patientId: number; prescribedOn: string | Date; diagnosis: string | null; medicines: string; instructions: string | null };

export default function PrescriptionForm({ patients, initialPatientId, initialVisit, editingPrescription }: { patients: Patient[]; initialPatientId?: number; initialVisit?: string; editingPrescription?: EditingPrescription }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch(editingPrescription ? `/api/prescriptions/${editingPrescription.id}` : "/api/prescriptions", { method: editingPrescription ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
      const body = await response.json(); if (!response.ok) throw new Error(body.error);
      toast.success(editingPrescription ? "Prescription updated." : "Prescription saved to the selected appointment date.");
  ...
  return (
    <form onSubmit={submit} className="clinic-workflow-form space-y-6 rounded-3xl border bg-white p-6 shadow-sm">
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <label className="space-y-2 text-sm font-medium">Completed appointment date<select required name="prescribedOn" defaultValue={editingPrescription ? dateKey(editingPrescription.prescribedOn) : initialVisit} disabled={completedAppointments.length === 0} className="h-11 w-full rounded-xl border bg-background px-3 disabled:bg-muted">{completedAppointments.length === 0 ? <option value="">No completed appointments</option> : completedAppointments.map((appointment) => <option key={appointment.id} value={dateKey(appointment.appointmentDate)}>{formatVisit(appointment)}</option>)}</select></label>
      </div>
      ...
      <label className="block space-y-2 text-sm font-medium">Diagnosis (optional)<Input name="diagnosis" defaultValue={editingPrescription?.diagnosis ?? ""} placeholder="Clinical diagnosis or treatment indication" /></label>
      <label className="block space-y-2 text-sm font-medium">Medicines<span className="block text-xs font-normal text-muted-foreground">One medicine per line: name - dose - frequency - duration</span><Textarea required name="medicines" defaultValue={editingPrescription?.medicines ?? ""} rows={7} placeholder={"Amoxicillin 500 mg - 1 tablet - twice daily - 5 days\nParacetamol 650 mg - 1 tablet - as needed - 3 days"} /></label>
      <label className="block space-y-2 text-sm font-medium">Patient instructions (optional)<Textarea name="instructions" defaultValue={editingPrescription?.instructions ?? ""} rows={4} placeholder="After-care, food advice, or when to contact the clinic" /></label>
      <div className="flex justify-end gap-3 border-t pt-5"><Button type="button" variant="outline" onClick={() => router.back()}>Cancel</Button><Button type="submit" disabled={!canSave}>{saving ? "Saving..." : editingPrescription ? "Update prescription" : "Save prescription"}</Button></div>
    </form>
  );
}
