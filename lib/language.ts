...
const languages: Record<string, ClinicLanguage> = { LANG_EN: "en", LANG_HI: "hi", LANG_MR: "mr" };
...
export async function welcomeFor(language: ClinicLanguage) {
  ...
  const configured = language === "hi" ? clinic?.whatsapp?.welcomeHindi : language === "mr" ? clinic?.whatsapp?.welcomeMarathi : clinic?.whatsapp?.welcomeEnglish;
